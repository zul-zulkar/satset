...
function renderFormSurat($judul) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!preg_match("/^[a-zA-Z'\s]+$/u", $nama)) {
            ...
            $stmtN = $mysqli->prepare("SELECT COALESCE(MAX(nomor), 0) AS maxn FROM antrian WHERE tanggal = ? AND jenis = 'surat'");
    ...
    <head>
        ...
        <style>
            ...
            .radio-label input[type=radio] { accent-color:#d97706; width:1rem; height:1rem; flex-shrink:0; }
        </style>
    </head>
    ...
    <div class="w-full max-w-xl mx-auto bg-white p-6 sm:p-8 rounded-lg shadow">
        ...
        <?php if ($tampilkanForm): ?>
            ...
            <form method="POST" class="space-y-6 text-sm" id="formSurat" autocomplete="off" novalidate>
                ...
                <div>
                    ...
                    <input type="text" id="pekerjaan_lainnya_input" name="pekerjaan_lainnya_text"
                           ...
                           class="mt-2 w-full border border-gray-300 p-2 rounded focus:outline-none focus:ring-1 focus:ring-amber-500 <?= $oldPekerjaanPilihan === 'Lainnya' ? '' : 'hidden' ?>">
                ...
                <div>
                    ...
                    <button type="button" id="btn-tambah-data"
                            class="mt-3 text-amber-700 border border-amber-600 px-4 py-1.5 rounded text-sm hover:bg-amber-50 transition-colors">
                        + Tambah Data
                    </button>
                </div>

                <button type="submit"
                        class="bg-amber-600 hover:bg-amber-700 text-white w-full py-2.5 rounded font-semibold transition-colors">
                    Kirim Data
                </button>
            </form>
            ...
            function createRecord(data, dari, sampai) {
                ...
                div.innerHTML =
                    ...
                    '         value="' + escHtml(data) + '" ' +
                    '         class="w-full border border-gray-300 p-2 rounded text-sm focus:outline-none focus:ring-1 focus:ring-amber-500">' +
                    '</div>' +
            ...
            document.getElementById('formSurat').addEventListener('submit', function(e) {
                ...
                if (!instansi) { alert('Asal instansi / organisasi wajib diisi.'); e.preventDefault(); return; }
                ...
                if (dataNamas.length === 0) { alert('Tambahkan minimal 1 data yang dibutuhkan.'); e.preventDefault(); return; }
                var hasData = false;
                for (var i = 0; i < dataNamas.length; i++) {
                    if (dataNamas[i].value.trim() !== '') { hasData = true; break; }
                }
                if (!hasData) { alert('Isi nama data pada minimal 1 baris data yang dibutuhkan.'); e.preventDefault(); return; }
        ...
        <?php else: ?>
            <div class="mt-6 p-8 bg-amber-50 border border-amber-400 rounded-lg text-center">
                <p class="text-4xl font-bold text-amber-700 mb-2">&#10003;</p>
                <p class="text-2xl font-bold text-amber-700">Terima kasih!</p>
