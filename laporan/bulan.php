<?php
/**
 * Dashboard Laporan Bulanan PST
 * Mencakup seluruh hari kerja dalam satu bulan,
 * dengan ringkasan per pekan dan akumulasi keseluruhan.
 */
...
foreach ($weeksInMonth as $w) {
    $pCnt = 0; $nCnt = 0;
    foreach ($pstRows    as $r) if ($r['tanggal'] >= $w['monday'] && $r['tanggal'] <= $w['friday']) $pCnt++;
    foreach ($nonPstRows as $r) if ($r['tanggal'] >= $w['monday'] && $r['tanggal'] <= $w['friday']) $nCnt++;
    $visitByWeek[$w['num']] = ['pst' => $pCnt, 'non' => $nCnt];
}

// Kunjungan per hari (untuk chart harian)
$visitByDay = [];
$dtCur = new DateTime($firstDay);
$dtEnd = new DateTime($lastDay);
while ($dtCur <= $dtEnd) {
    $dow = (int)$dtCur->format('N');
    if ($dow <= 5) { // Senin–Jumat
        $d = $dtCur->format('Y-m-d');
        $pCnt = count(array_filter($pstRows,    fn($r) => $r['tanggal'] === $d));
        $nCnt = count(array_filter($nonPstRows, fn($r) => $r['tanggal'] === $d));
        $visitByDay[$d] = ['pst' => $pCnt, 'non' => $nCnt];
    }
    $dtCur->modify('+1 day');
}
...
<main id="reportContent" class="max-w-7xl mx-auto px-4 pt-14 lg:pt-6 pb-6 space-y-6">
  ...
  <div class="flex items-start gap-4">
    <div>
      <h1 class="text-xl font-bold text-gray-800">Laporan Bulanan Pelayanan PST</h1>
      <p class="text-sm text-gray-500 mt-0.5">
        <i class="fas fa-calendar-alt text-blue-500 mr-1.5"></i><?= htmlspecialchars($periodLabel) ?>
        — <?= (new DateTime($firstDay))->format('d M') ?> s.d. <?= (new DateTime($lastDay))->format('d M Y') ?>
      </p>
    </div>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
      <i class="fas fa-table-cells text-indigo-500"></i>
      <h2 class="font-semibold text-gray-700">Ringkasan per Pekan</h2>
      <span class="ml-auto text-xs text-gray-400"><?= count($weeksInMonth) ?> pekan</span>
    </div>
    <div class="overflow-x-auto">
      <table class="w-full text-sm text-center">
        <thead>
          <tr class="bg-gray-50">
            <th class="px-4 py-2 text-left font-semibold text-gray-600">Pekan</th>
            <th class="px-4 py-2 text-left font-semibold text-gray-500 text-xs">Rentang</th>
            <th class="px-4 py-2 font-semibold text-blue-600">PST</th>
            <th class="px-4 py-2 font-semibold text-slate-500">Non-PST</th>
            <th class="px-4 py-2 font-semibold text-gray-700">Total</th>
            <th class="px-4 py-2 font-semibold text-gray-400 text-xs no-print">Aksi</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-50">
          <?php foreach ($weeksInMonth as $w):
            $vw  = $visitByWeek[$w['num']];
            $tot = $vw['pst'] + $vw['non'];
            $wUrl = APP_BASE . "/laporan/minggu?tahun=$selYear&bulan=$selMonth&pekan={$w['num']}";
          ?>
          <tr class="hover:bg-indigo-50/30">
            <td class="px-4 py-2.5 text-left">
              <span class="font-semibold text-gray-700">Pekan <?= $w['num'] ?></span>
            </td>
            <td class="px-4 py-2.5 text-left text-xs text-gray-400">
              <?= (new DateTime($w['monday']))->format('d M') ?> – <?= (new DateTime($w['friday']))->format('d M') ?>
            </td>
            <td class="px-4 py-2.5 font-semibold text-blue-700"><?= $vw['pst'] ?: '—' ?></td>
            <td class="px-4 py-2.5 text-slate-500"><?= $vw['non'] ?: '—' ?></td>
            <td class="px-4 py-2.5 font-bold text-gray-800"><?= $tot ?: '—' ?></td>
            <td class="px-4 py-2.5 no-print">
              <a href="<?= htmlspecialchars($wUrl) ?>" target="_blank"
        ...
        <tfoot>
          <tr class="bg-blue-50 font-bold">
            <td class="px-4 py-2.5 text-left text-blue-700" colspan="2">Total Bulan</td>
            <td class="px-4 py-2.5 text-blue-700"><?= $totalPST ?></td>
            <td class="px-4 py-2.5 text-slate-600"><?= $totalNonPST ?></td>
            <td class="px-4 py-2.5 text-gray-800"><?= $totalPST + $totalNonPST ?></td>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
      ...
      <h2 class="font-semibold text-gray-700">Grafik Kunjungan Harian</h2>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
      ...
      <h2 class="font-semibold text-gray-700">Rekap Kehadiran Piket</h2>
      <span class="ml-auto text-xs text-gray-400"><?= $totalPiket ?> petugas hadir bulan ini</span>
    </div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
            ...
            <th class="px-3 py-3 text-left font-semibold">Tanggal Hadir</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-50">
          ...
          <tr class="hover:bg-teal-50/30">
            <td class="px-4 py-2.5 font-medium text-gray-800"><?= htmlspecialchars($pg['nama']) ?></td>
            <td class="px-4 py-2.5 text-gray-400 text-xs"><?= htmlspecialchars($pg['jabatan']) ?></td>
            <td class="px-3 py-2.5 text-center">
              <span class="inline-block px-2 py-0.5 rounded-full text-xs font-bold
                <?= $hadir >= 18 ? 'bg-teal-100 text-teal-700' : ($hadir >= 10 ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500') ?>">
                <?= $hadir ?> hari
              </span>
            </td>
            <td class="px-3 py-2.5 text-xs text-gray-500">
              <?= implode(', ', array_map(fn($d) => (new DateTime($d))->format('d/m'), array_keys($pg['days']))) ?>
            </td>
          </tr>
          <?php endforeach; ?>
        </tbody>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <?php if (!$totalPST): ?>
      <div class="px-5 py-8 text-center text-gray-400 text-sm">Belum ada pengunjung PST bulan ini.</div>
      <?php else: ?>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div class="px-5 py-8 text-center text-gray-400 text-sm">Belum ada data yang tercatat bulan ini.</div>
      <?php else: ?>
      <div class="p-5">
        <ul class="space-y-2">
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <?php if (!$totalSurvei): ?>
    <div class="px-5 py-8 text-center text-gray-400 text-sm">Belum ada survei bulan ini.</div>
    <?php else: ?>
    <div class="p-5">
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <?php if (!$totalPES): ?>
    <div class="px-5 py-8 text-center text-gray-400 text-sm">Belum ada PES bulan ini.</div>
    <?php else: ?>
    <div class="p-5">
        ...
        <div>
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center gap-3 px-3 py-2.5 rounded-xl <?= $cls ?>">
              ...
              <div><p class="font-semibold text-sm"><?= $sentiLabel[$s] ?></p></div>
...
<script>
(function() {
  const labels = <?= json_encode(array_map(fn($d) => (new DateTime($d))->format('d/m'), array_keys($visitByDay))) ?>;
  const pst    = <?= json_encode(array_map(fn($v) => $v['pst'], array_values($visitByDay))) ?>;
  const nonPst = <?= json_encode(array_map(fn($v) => $v['non'], array_values($visitByDay))) ?>;
  ...
  new Chart(document.getElementById('chartVisit'), {
    ...
    data: {
      labels,
      datasets: [
        { label: 'PST',     data: pst,    backgroundColor: '#3b82f680', borderColor: '#3b82f6', borderWidth: 1.5, borderRadius: 3 },
        { label: 'Non-PST', data: nonPst, backgroundColor: '#94a3b840', borderColor: '#94a3b8', borderWidth: 1.5, borderRadius: 3 },
      ]
    },
    options: {
...
async function downloadPDF() {
  ...
  try {
    await html2pdf()
      .set({
        margin:      [14, 10, 14, 10],
        filename:    'laporan-bulanan-<?= $selYear ?>-<?= sprintf('%02d',$selMonth) ?>.pdf',
